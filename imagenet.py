...
model_names = sorted(name for name in resnet_image.__dict__
    if name.islower() and not name.startswith("__")
                    and name.startswith("resnet")
                    and callable(resnet_image.__dict__[name]))
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet18',
                    help='model architecture: ' + ' | '.join(model_names) +
                    ' (default: resnet50)')
parser.add_argument('--data-path', default='path to dataset', type=str)
...
def main():
    ...
    #     torch.save(model.state_dict(), "./{}/{}_{}_acc_{}_sgd.pt".format(
    #         args.save_dir, args.dataset, args.arch, best_prec1))
